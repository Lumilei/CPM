...
def MNA(emb, item_list, candidate_emb, index, time_threshold):
    """多节点聚合：计算满足时间约束的嵌入数量"""
    count = 0
    for sequence in emb:
        stop = 0
...
def create_Lk(Ck, L, G, sup_g, subg_set, support, candidate_emb, dia_threshold, time_threshold, sup_threshold, item_count_dict):
    """创建k-频繁项集"""
    Lk = set()
    Ck_list = list(Ck)
    for item_set in Ck_list:
        ...
        for g in subg_set:
            ...
            sup = MNA(embedding, item_list, candidate_emb, index, time_threshold)
            embedding.clear()
            if not success_flag:
...
def generate_L(L, L1, G, sup_g, subg_set, max_item, support, dia_threshold, time_threshold, sup_threshold, item_count_dict):
    """生成所有频繁项集"""
    Lksub1 = L1.copy()
    ...
    for i in range(2, max_item):
        ...
        Lk = create_Lk(Ck, L, G, sup_g, subg_set, support, candidate_emb, dia_threshold, time_threshold, sup_threshold, item_count_dict)
        Lksub1 = Lk.copy()
    return L, support
...
def main():
    ...
    L, support = generate_L(L, L1, g, sup_g, subg_set, max_item, support, 
                           dia_threshold, time_threshold, sup_threshold, item_count_dict)
    ...
    # 保存结果
    with open('result/delicious/picpm/support(50).txt', 'w') as f:
        for Lk in L.values():
            if not Lk:
                break
