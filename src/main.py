...
if __name__ == '__main__':
    ...
    dia_threshold = 3
    time_threshold = 4320000#72min
    sup_threshold = 50
    data_set = data_load("src/Delicious/datasets/delicious_activity_top3.txt")

    data_set, item_count_dict, node_id = attribute_count(data_set, min_sup=50)  # 单一属性阈值
    ...
    # 打开文件进行写入
    with open('result/delicious/new/support(50).txt', 'w') as f:
        for Lk in L.values():
            if not Lk:
                break
            ...
            for i in range(len(res)):
                if i < top_k:
                    print(list(res[i][0]))
                    print("11111:", list(res[i][0]))

                    # significance_test
                    g_score, p_value = Sign_Test.significance_test(g, list(res[i][0]), label_dict)
